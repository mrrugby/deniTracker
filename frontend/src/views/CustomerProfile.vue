<template>
  ...
  <div class="balance highlight">
    ...
    <section class="transactions">
      ...
      <div v-if="transactions.length" v-for="t in transactions" :key="t.id" class="transaction-card" @click="openTransactionDetails(t)">
...
>
  <div class="modal" @click.stop>
    ...
    <div v-if="selectedTransaction">
      ...
      <p><strong>Type:</strong>
        {{ selectedTransaction.transaction_type === 'debt' ? 'Credit Sale' : 'Payment' }}
      </p>
   ...
   : Number(selectedTransaction.amount || 0).toLocaleString(undefined,{minimumFractionDigits:2})
}} Ksh
      </p>
  ...
  minimumFractionDigits:2
      ...
      <div class="modal-actions" style="margin-top:1.2rem">
        <button class="btn cancel" @click="showTransactionDetailModal=false">
          Close
        </button>
      </div>
    ...
    <div v-if="showPaymentModal || showDebtModal" class="modal-backdrop" @click="closeModals">
      <div class="modal" @click.stop>
        ...
        <div v-if="showPaymentModal">
          ...
          <input v-model.number="paymentAmount" type="number" placeholder="Enter amount" autofocus />
          <div class="modal-actions">
            <button @click="submitPayment" class="btn primary">Save</button>
...
import { fetchCustomer, fetchItems, addTransaction, calculateCustomerBalance  } from "@/services/local";
import TopNav from "@/components/TopNav.vue";
import BottomNav from "@/components/BottomNav.vue";

const route = useRoute();

// Initialize customer safely
const customer = ref({ id: 0, name: "", total_debt: 0, total_payments: 0, transactions: [] });

// Extract transactions as a computed property
  ...
  return (transactions.value || [])
    ...
    .reduce((sum, t) => {
      if (t.items && t.items.length) {
        return sum + t.items.reduce((s, i) => s + i.unit_price * i.quantity, 0);
      }
      return sum;
    }, 0);
});
...
const outstanding = computed(() => totalDebt.value - totalPaid.value);
...
async function submitPayment() {
  if (!paymentAmount.value) return alert("Enter an amount");
...
async function submitDebt() {
  ...
  await addTransaction(customer.value.id, "debt", {
    amount: totalAmount,
    items: selected
  });
